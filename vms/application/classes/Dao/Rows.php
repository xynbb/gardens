 ...
 * 数据库操作类 —— 园
 */
class Dao_Rows extends Dao {
	protected $_db = 'account';

	protected $_tableName = 'rows';
	...
	public function getRowsByKeyword($condition, $number = 0, $offset = 0) {
		...
		return $select->as_object('Model_Rows')->execute($this->_db);
	}
	 ...
	 * @return array
	 */
	public function getRowsByAreaId($areaId) {
		return DB::select('*')
			->from($this->_tableName)
			->where('area_id', '=', $areaId)
			->as_object('Model_Rows')
			->execute($this->_db);
	}
