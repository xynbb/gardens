<ul class="pagination">
  ...
  <?php
    ...
    $max = ($pagination->max < 500) ? $pagination->max : 500;
    $begin = ($previousPass > 0) ? $previousPass : 1;
    $end = ($nextPass < $max) ? $nextPass : $max;
    while($begin <= $end) {
    ...
    if($pagination->current == $max) {
      ...
      echo '<li><a href="'.$pagination->URL($max).'"><i class="glyphicon glyphicon-step-forward"></i></a></li>';
    }
  ?>
</ul>
